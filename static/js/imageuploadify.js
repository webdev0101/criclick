...
;(function($, window, document, undefined) {
  ...
  $.fn.imageuploadify = function(opts) {
    ...
    this.each(function() {
      ...
      let counter = 0;
	 //Dragbox 1st step// Layout
		let openDragbox = $(`
		  <div class="open-dragbox">
			<div class="open-dragbox-icon">
				<i class="fal fa-camera"></i>
			</div>
			<div class="open-dragbox-msg">
				<span>Post with photos get more views</span>
      ...
      let dragbox = $(`
      <div class="imageuploadify well hidden">
        <div class="imageuploadify-overlay">
        <i class="fa fa-picture-o"></i>
        </div>
        <div class="imageuploadify-images-list text-center">
		  <div class="imageuploadify-inner">
          <span class='imageuploadify-message'>Drag and Drop your Photos</span>
          <button type="button" class="cummon-button-md btn btn-primary" >Select Photos</button>
		  </div>
        </div>
		<div class="imageuploadify-btn hidden">
			  <button type="button" class="cummon-button-md btn btn-primary cancel-btn">Cancel</button>
		  <button type="button"  class="cummon-button-md btn btn-primary upload-btn">Upload Photos</button>
		</div>
      </div>
      `);
      ...
      let imageuploadifyinner = dragbox.find(".imageuploadify-images-list .imageuploadify-inner");
	  let imageButton = dragbox.find(".imageuploadify-btn");


	  let cancelBtn = dragbox.find(".cancel-btn");
	  let uploadBtn = dragbox.find(".upload-btn");
      ...
      const retrieveFiles = (files) => {
        ...
        for(let index = 0; index < files.length; ++index) {
          if (!accept || compareMimeType(result, files[index].type, /(?:\.([^.]+))?$/.exec(files[index].name)[1])) {
			...
			console.log(id+' '+totalFiles.length);
			if(totalFiles.length>=4){
				console.log('Please upload maximun 4 images.');
				return false;
			}
      ...
      const readingFile = (id, file) => {
		...
		// Create option box for add more photo.
        let addMorePhoto = $(`<div class='imageuploadify-container add-more-photo'>	
			<div class="add-more-photo-icon"><i class="fal fa-plus"></i></div>
        </div>`);
		let addMore = addMorePhoto.find(".add-more-photo");

        // Create the preview file container box.
        let container = $(`<div class='imageuploadify-container'>
          <button type='button' class='btn btn-danger fal fa-trash-alt hidden'></button>
		...
		let elemBar = container.find(".myBar");
        ...
        if (file.type && file.type.search(/image/) != -1) {
          ...
          fReader.onloadend = function (e) {
			...
			imagesList.find('.add-more-photo').remove();
			if(totalFiles.length<4){
				imagesList.find('.imageuploadify-container:last-child').after(addMorePhoto);
			}else{
				imagesList.find('.imageuploadify-container').after(addMorePhoto.addClass('hidden'));
			}
		...
		if(totalFiles.length>=0){
			imageButton.removeClass('hidden');
		}
		addMorePhoto.on("click", function(event) {
			event.stopPropagation();
			event.preventDefault();
			
			imageButton.removeClass('hidden');
			imageuploadifyinner.removeClass('hidden');
			$(self).click();
		});	
        ...
        deleteBtn.on("click", function() {
		  ...
		  addMorePhoto.removeClass('hidden');
		  /*if(totalFiles.length<4){
			imagesList.find('.add-more-photo').remove();
			imagesList.find('.imageuploadify-container:last-child').after(addMorePhoto);
		  }*/
	  ...
	  cancelBtn.on("click", function(e) {
			dragbox.find(".imageuploadify-container").remove();
			totalFiles = [];
			imageButton.addClass('hidden');
      });
	  uploadBtn.on("click", function(e) {
			imageuploadifyinner.addClass('hidden');
			imageButton.addClass('hidden');
      });
      ...
      button.on("click", function onClick(event) {
	   ...
	   /*imageuploadifyinner.on("click", function onClick(event) {
        event.stopPropagation();
        event.preventDefault();
        $(self).click();
      });*/
        ...
        $(window).resize(function () {
          ...
          window.resizeEvt = setTimeout(function() {
            ...
            for (let index = 0; index < containers.length; ++index) {
              $(containers[index]).css("margin-left", "0px");
              $(containers[index]).css("margin-right", marginSize + "px");
            }

            // Apply left margin to first container of each row and right to last.
            imagesList.find(".imageuploadify-container:nth-child(" + boxesNb + "n+4)").css("margin-right", marginSize + "px");
            imagesList.find(".imageuploadify-container:nth-child(" + boxesNb + "n+3)").css("margin-left", marginSize + "px");
          }, 500);
        });
      })
     ...
     /* $(self).closest("form").on("submit", function(event) {
		...
		//Open Drag Box
	  openDragbox.on("click", function(){
		dragbox.removeClass("hidden");
